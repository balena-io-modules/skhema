...
// adds any custom formats added using `skhema.addFormat`.
// This method mutates the AJV instance.
const augmentAjv = (ajv) => {
	const keywords = [
		'formatMaximum',
		'formatMinimum'
	]
	...
	keywords.forEach((keyword) => {
		if (!ajv.RULES.keywords) {
			ajvKeywords(ajv, [ keyword ])
		}
	})

	_.forEach(customFormats, (callback, format) => {
 ...
 * @description Add a custom format type, along with a validation function.
 * @function
 * @public
 *
 * @param {String} format - The name of the format
 * @param {formatValidationCallback} callback - The format validation callback
 *
 * @example
 ...
 * })
 */
exports.addFormat = (format, callback) => {
	customFormats[format] = callback
}
...
/**
 * @summary Validates a schema against the schema-of-schemas
 * @description Validates a schema against the schema-of-schemas. Will return
 * true if the schema is valid and false otherwise
 * @function
 * @private
 *
 * @param {Object} ajv - ajv instance to supply
 ...
 * })
 */
const isValidSchema = (ajv, schema) => {
	return ajv.validate('schema', schema) || schema.type
}
 ...
 *
 * @description
 * If an array of schemas is supplied, the last schema will be used to
 * validate the object.
 *
 * @param {Object|Object[]} schema - JSON schema
...
exports.match = (schema, object) => {
	...
	// Augment ajv again to pick up any new formats that have been added
	augmentAjv(ajv)
	...
	// The last schema supplied is the one we run validate against
	const objSchema = _.last(schemas)

	// If we have more than one schema we add them to ajv in order
	if (schemas.length > 1) {
		const additionalSchemas = _.take(schemas, schemas.length - 1)
		for (const additionalSchema of additionalSchemas) {
			ajv.addSchema(additionalSchema)
			if (!isValidSchema(ajv, additionalSchema)) {
				return {
					valid: false,
	...
	if (!isValidSchema(ajv, objSchema)) {
		return {
			valid: false,
			errors: [ 'invalid schema' ]
		}
	...
	const valid = ajv.validate(objSchema, object)
