...
/**
 * @summary Create a partial to filter an object based on a schema
 * @function
 * @public
 *
 * @param {Object} schema - schema
 * @returns {Function} function to filter an object
 *
 * @example
 * const result = skhema.createPartialFilter({
 *	 type: 'object',
 *	 properties: {
 *		 foo: {
 *			 type: 'number'
			...
			for (const item of items) {
				...
				if (matchers.length === 0) {
					...
					if (filterSchema.anyOf) {
						return exports.filter(filterSchema, items)
					}
...
exports.filter = (schema, object) => {
	...
	const parse = exports.createPartialFilter(schema)
