...
/**
 * @summary Prune invalid anyOf branches from a schema
 * @function
 * @public
 *
 * @description This function mutates `schema`
 *
 * @param {Object} schema - schema
 * @param {Object} object - object
 * @returns {Object} pruned schema
 *
 * @example
 * const result = skhema.prune({
 *	 type: 'object',
 *	 anyOf: [
 *     {
 *       type: 'object',
 ...
 * > }
 */
exports.pruneAnyOf = (schema, base) => {
	if (!schema) {
	...
	schema.anyOf = schema.anyOf.filter((branch) => {
		...
		if (branch.anyOf) {
			exports.pruneAnyOf(branch, schema)
		}

		for (const key in branch.properties) {
			const branchConst = branch.properties[key].const
			const branchEnum = branch.properties[key].enum
			if (
				schema.properties &&
				...
				if (branchConst) {
					if (
						typeof schema.properties[key].const !== 'undefined' &&
						schema.properties[key].const !== branchConst
					) {
						return false
					}
					...
					if (
						...
						schema.properties[key].enum.indexOf(branchConst) === -1
					) {
						return false
					}
				}
				...
				if (branchEnum) {
					if (
						typeof schema.properties[key].const !== 'undefined' &&
						branchEnum.indexOf(schema.properties[key].const) === -1
					) {
						return false
					}
			...
			if (
				...
				if (branchConst) {
					if (
						typeof base.properties[key].const !== 'undefined' &&
						base.properties[key].const !== branchConst
					) {
						return false
					}

					if (
						typeof base.properties[key].enum !== 'undefined' &&
						base.properties[key].enum.indexOf(branchConst) === -1
					) {
						return false
					}
				}
				if (branchEnum) {
					if (
						typeof base.properties[key].const !== 'undefined' &&
						branchEnum.indexOf(base.properties[key].const) === -1
					) {
						return false
					}
					...
					if (
						typeof base.properties[key].enum !== 'undefined' &&
						_.intersection(branchEnum, base.properties[key].enum).length === 0
					) {
						return false
					}
				}
...
exports.filter = (() => {
	...
	return (schema, object, options = {}) => {
		...
		const parse = (thisSchema) => {
			const preparedSchema = exports.pruneAnyOf(
				exports.normaliseRequires(
