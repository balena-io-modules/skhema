...
exports.filter = (() => {
	...
	return (schema, object, options = {}) => {
		...
		if (_.isUndefined(preparedSchema.additionalProperties)) {
			preparedSchema.additionalProperties = false
		}

		// Parse the schema to create a schema that will be used for filtering
		// properties
		const filterValidator = filterAjv.compile(disallowAdditionalProperties(
			_.cloneDeep(preparedSchema)
		))
