...
exports.filter = (() => {
	...
	/**
	 * @summary Compiles schema checking the cache
	 * @function
	 * @private
	 *
	 * @param {String} ajvInstance - Either 'filter' or 'match', picks the correct ajvInstance
	 * @param {Object} schema - the schema to compile
	 * @returns {Function} result of ajvInstance.compile(schema)
	 *
	 * @example
	 * compileSchemaCached(ajv, {
	 *	type: 'object',
	 *	properties: {
	 *		foo: {
	 *			type: 'number'
	 ...
	 *
	 */
	const compileSchemaCached = (ajvInstance, schema) => {
		...
		const found = CACHE.get(`${ajvInstance}-${hash}`)
		if (found) {
			return found
		}
		let fn = null
		if (ajvInstance === 'filter') {
			fn = filterAjv.compile(schema)
		} else {
			fn = matchAjv.compile(schema)
		}
		CACHE.set(`${ajvInstance}-${hash}`, fn)
		return fn
	}
	...
	return (schema, object, options = {}) => {
		...
		const parse = (thisSchema) => {
			...
			// properties
			const filterValidator = compileSchemaCached(
				'filter',
				preparedSchema
			)
			...
			if (preparedSchema.anyOf) {
				fragmentValidators = _.map(preparedSchema.anyOf, (fragment) => {
					...
					const matchFn = compileSchemaCached(
						'match',
			...
			for (const item of items) {
				...
				if (_.isEmpty(matchers)) {
					...
					const mergedFilterValidator = compileSchemaCached(
						'filter',
						filterSchema
					)
