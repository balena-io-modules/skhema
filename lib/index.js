...
exports.filter = (() => {
	...
	return (schema, object, options = {}) => {
		...
		const preparedSchema = exports.normaliseRequires(schema)

		// Parse the schema to create a schema that will be used for filtering
		// properties
		const filterValidator = filterAjv.compile(
			disallowAdditionalProperties(_.cloneDeep(preparedSchema), options))
...
const makeFilter = (baseSchema, matchers) => {
	...
	return disallowAdditionalProperties(mergedFragment)
}
 ...
 * @param {Function} combineAdditionalProperties - function describing how to merge additionalProperties
 * @returns {Object} merged schema
 *
 * @example
 ...
 * }
 */
const combineSchemas = (left, right, combineAdditionalProperties) => {
	...
	return _.omit(_.mergeWith(
		...
		(objectValue, sourceValue, key) => {
			if (_.isArray(objectValue)) {
				return _.union(objectValue, sourceValue)
			}
			if (key === 'additionalProperties') {
				return combineAdditionalProperties(objectValue, sourceValue)
			}
		}
	), [ '$id' ])
}
