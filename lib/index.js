...
const allowAdditionalProperties = (schema) => {
	...
	schema.properties = _.mapValues(schema.properties,
		allowAdditionalProperties)

	if (schema.anyOf) {
		schema.anyOf = _.map(schema.anyOf, allowAdditionalProperties)
	}

	if (schema.allOf) {
		schema.allOf = _.map(schema.allOf, allowAdditionalProperties)
	}
	...
	return (schema, object, options = {}) => {
		...
		if (schema.anyOf) {
			...
			fragmentValidators = _.map(schema.anyOf, (fragment) => {
				...
				const match = matchAjv.compile(
					allowAdditionalProperties(_.cloneDeep(mergedFragment)))
